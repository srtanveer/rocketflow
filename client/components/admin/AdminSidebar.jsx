    ...
    <Card className={cn('h-screen py-4 px-3 flex flex-col transition-width', collapsed ? 'w-20' : 'w-72', className)} hover={false} padding="md">
      <div className="flex items-center justify-between mb-4">
        <div className={cn("flex items-center gap-3 transition-all", collapsed ? 'justify-center w-full' : '')}>
          <div className={cn("w-10 h-10 rounded-full bg-primary flex items-center justify-center text-white font-bold text-lg")}>RF</div>
          {!collapsed && (
            <div>
              <div className="font-bold text-base text-primary">RocketFlow</div>
...
function SidebarLink({ href, icon, label, collapsed }) {
  return (
    <a
      href={href}
      className={cn(
        "flex items-center gap-3 px-3 py-2 rounded-md font-medium transition-colors hover:bg-blue-50 hover:text-primary",
        "text-gray-700"
      )}
      title={label}
    >
