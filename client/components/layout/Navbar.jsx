                    ...
                    >
                      ...
                      {(item.name === 'Industries' && isIndustriesDropdownOpen) && (
                        <>
                          ...
                          <div className="fixed left-0 right-0 h-2 z-40" style={{ top: '3.5rem' }} />
                          
                          <div 
                            className="fixed left-0 right-0 bg-white shadow-2xl border border-gray-200 z-50 rounded-3xl mx-4" 
                            style={{ top: '4rem' }}
                            onMouseEnter={handleMouseEnter}
                            onMouseLeave={handleMouseLeave}
                          >
                          {item.dropdownColumns ? (
                            ...
                            <div className="grid grid-cols-4 gap-x-8 py-8 px-16 max-w-7xl mx-auto">
                              {item.dropdownColumns.map((column, columnIndex) => (
                                ...
                                >
                                  <h3 className={`text-xs font-bold uppercase tracking-wider mb-2 pb-2 border-b ${column.title.includes('Coming soon') ? 'text-gray-400 border-gray-200' : 'text-blue-600 border-blue-200'}`}>
                                    {column.title}
                                  </h3>
                                  <div className={column.colSpan > 1 ? 'grid grid-cols-2 gap-x-8 gap-y-0.5' : 'space-y-0.5'}>
                                    {column.items.map((dropdownItem) => (
                                      <a
                                        ...
                                        className={`block px-3 py-1.5 text-base font-semibold rounded-md transition-colors duration-200 leading-snug ${column.title.includes('Coming soon') ? 'text-gray-400 hover:text-gray-500 hover:bg-gray-50' : 'text-gray-800 hover:text-blue-600 hover:bg-blue-50'}`}
                                      >
                                        {dropdownItem.name}
                                      </a>
                                    ))}
                        ...
                        {item.dropdownColumns ? (
                          ...
                          item.dropdownColumns.map((column) => (
                            <div key={column.title} className="mt-2">
                              <h3 className={`text-xs font-semibold uppercase tracking-wider mb-2 px-3 ${column.title.includes('Coming soon') ? 'text-gray-400' : 'text-blue-600'}`}>
                                {column.title}
                              </h3>
                              <div className="grid grid-cols-2 gap-1">
                                {column.items.map((dropdownItem) => (
                                  <a
                                    key={dropdownItem.name}
                                    href={dropdownItem.href}
                                    className={`block px-2 py-2 text-xs sm:text-sm font-medium transition-colors duration-200 rounded-lg ${column.title.includes('Coming soon') ? 'text-gray-400 hover:text-gray-500 hover:bg-gray-50' : 'text-gray-600 hover:text-blue-600 hover:bg-white/40'}`}
